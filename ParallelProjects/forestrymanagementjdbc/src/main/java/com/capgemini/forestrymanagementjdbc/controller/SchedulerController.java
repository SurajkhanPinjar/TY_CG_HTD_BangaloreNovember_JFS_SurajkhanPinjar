...
public class SchedulerController {
	...
	public static void schedulerController() {
		...
		while (schedulerLoop == 'a') {
			...
			case 1:
				...
				schedulerBean.setSchedulerId(schedulerIdForadd);
				...
				while (schedulerPasswordLoopForAdd == 'y') {
					...
					if (strength == 10) {
						...
						schedulerBean.setSchedulerPassword(schedulerPasswordValidation);
			...
			case 2:
				...
				schedulerBean.setSchedulerId(schedulerIdForDelete);

				System.out.println("Enter The Password :");
				schedulerBean.setSchedulerPassword(scanner.nextLine());

				try {
					if (schedulerServices.deleteScheduler(schedulerBean.getSchedulerId(), schedulerBean.getSchedulerPassword())) {
				...
				try {
					...
					for (SchedulerBean schedulerBean03 : allSchedulerList) {
						if (allSchedulerList != null) {
							System.out.println("Scheduler Id :" + schedulerBean03.getSchedulerId());
							System.out.println("Schedular Name :" + schedulerBean03.getSchedulerName());
							System.out.println("------------------------");
						}
					}
	...
	public static void loginScheduler() {
		...
		while (true) {
			...
			schedulerBean.setSchedulerId(schedulerLoginChoice);

			System.out.println("Enter The Password :");
			schedulerBean.setSchedulerPassword(scanner.nextLine());
			if (schedulerBean.getSchedulerPassword().equalsIgnoreCase("exit")) {
				SchedulerController.schedulerController();
			}

			try {
				if (schedulerService.loginScheduler(schedulerBean.getSchedulerId(), schedulerBean.getSchedulerPassword())) {
