	...
	public static void clientSection() {
		...
		while (clientSectionLoop == 'a') {
			...
			case 1:
				...
				while (clientPasswordLoopForAdd == 'y') {
					...
					if (strength == 10) {
						...
						clientBean.setClientPassword(clientPasswordValidation);
			...
			case 2:
				...
				if (clientBean.getClientPassword().equalsIgnoreCase("exit")) {
					ClientSection.clientSection();
				}

				try {
					if (customerServices.deleteClient(clientBean.getClientId(), clientBean.getClientPassword())) {
