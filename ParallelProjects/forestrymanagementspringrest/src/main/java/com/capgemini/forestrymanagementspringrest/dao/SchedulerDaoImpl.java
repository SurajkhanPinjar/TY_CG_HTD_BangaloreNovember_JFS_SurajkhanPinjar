...
public class SchedulerDaoImpl implements SchedulerDao {
	...
	public Scheduler login(Scheduler scheduler) {
		...
		String jpql = "from Scheduler where schedulerId=:id ";
		TypedQuery<Scheduler> query = manager.createQuery(jpql, Scheduler.class);
		query.setParameter("id",scheduler.getSchedulerId());
		try {
			Scheduler schedulerAccount = query.getSingleResult();
			if (schedulerAccount.getSchedulerPassword().equals(scheduler.getSchedulerPassword()) && 
					schedulerAccount.getRole().equalsIgnoreCase("scheduler")) {
				return schedulerAccount;
			} 
		} catch (Exception e) {
		}
		throw new AdminExceptions("Invalid Scheduler Account");
	}
