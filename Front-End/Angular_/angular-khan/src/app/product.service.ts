...
export class ProductService {
  ...
  deleteData(product: Product): Observable<any> {
    return this.http.delete<any>(
      `${this.api}/delete-product/${product.id}`
    );
  }

  updateData(product): Observable<any> {
  //  this one for using @PathVariable
    // return this.http.put<any>(
    //   `${this.api}/modify-product/${product.id}`, product
    // );

    return this.http.put<any>(`${this.api}/modify-product`,product);
  }
}
